...
class SpeechBuilder {
    ...
    /**
     * Adds a sub tag with an alias. The alias can either be a string or an object
     * with the platform type as key (AlexaSkill or GoogleAction).
     * @public
     * @param {string} text
    ...
    addSub(text, alias) {
        if (_.isObject(alias)) {
            alias = alias[this.jovo.getType()];
        };
        if (!_.isString(alias)) return this.addText(text);
        return this.addText(`<sub alias="${alias}">${text}</sub>`);
    }
